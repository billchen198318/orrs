...
INSERT INTO `tb_orrs_command` VALUES ('2defe1af-907a-11ef-9c7f-bb129a46f93f','test002','輸出資料轉為html','測試用的','給我HTML code, 把下列json內容轉成html <tr> <td> 展示出來, 資料如下:\n```json\n$P{previousInvokeResult}\n```\n資料列 各 td 內容為 json 項目的資料值, 給我HTML code ','result','HTML','gemma2','admin','2024-10-22 21:33:10','admin','2024-10-30 22:27:03'),('6fef7cd7-8e18-11ef-ad17-4132272e35db','test001','資料庫代碼','測試用的','mariadb 帳戶 root 密碼 password , 資料表 tb_sys_event_log 欄位 [OID, USER, SYS_ID, EXECUTE_EVENT] OID字串格式,USER字串格式,SYS_ID字串格式,EXECUTE_EVENT字串格式 , 用 groovy 產出連線資料庫取出資料, 先將資料放入List<Map> 中,  再用com.fasterxml.jackson 將 List<Map> 內容轉成 json 放到jsonResult變數, 程式最後 return  jsonResult, 給我 groovy code','result','GROOVY','gemma2','admin','2024-10-19 20:48:28','admin','2024-11-03 15:24:48'),('974b7fff-9aaa-11ef-b609-a1eae7d5353c','PP01','取得水量拆分檔內容','','1. sql server 帳戶 sa 密碼 P@ssw0rd@@@@  \n2. 資料表 WATER_QUANTITY_SPLIT\n3. 資料表 WATER_QUANTITY_SPLIT欄位 [W_ID, QUANTITY, TOTAL_AMOUNT, CHARGE_SDATE, CHARGE_EDATE]\n4. 資料表 WATER_QUANTITY_SPLIT欄位型態 [W_ID(int), QUANTITY(decimal), TOTAL_AMOUNT(decimal), CHARGE_SDATE(datetime), CHARGE_EDATE(datetime)]\n5. 資料表關聯關係 \n	WATER_QUANTITY_SPLIT.FACTORY_WATER_SOURCES_ID1 = FACTORY_WATER_SOURCES.OID \n	FACTORY_WATER_SOURCES.FACTORY_ID = FACTORY_HISTORY.OID\n	COMPANY.OID = FACTORY_HISTORY.COMPANY_OID\n	WATER_SOURCES_HISTORY.OID = FACTORY_WATER_SOURCES.WATER_SOURCES_ID\n6. 資料表 COMPANY 欄位[F_NAMEC] , COMPANY資料表欄位型態 [ F_NAMEC(varchar2)]\n7. 資料表 WATER_SOURCES_HISTORY 欄位[ \"WATER_SOURCES_ID\" ] , WATER_SOURCES_HISTORY 資料表欄位型態 [ \"WATER_SOURCES_ID\" (varchar2)]\n8. 用 groovy 產出連線資料庫, 資料表要串聯關係, \n	並取出資料 WATER_QUANTITY_SPLIT,COMPANY,WATER_SOURCES_HISTORY 這3個資料表定義的欄位 前10筆(如: select top 10 * from ....) \n	條件: \n        a. CHARGE_SDATE 大於等於2023年2月1日 , 且 CHARGE_EDATE 小於等於2023年4月30日\n	b. FACTORY_HISTORY.BASE_ID 為字串 \'01\'\n	c. 以QUANTITY desc排序\n9. 先將資料放入List<Map> 中,  再用com.fasterxml.jackson 將 List<Map> 內容轉成 json 放到jsonResult變數,  \n10. 最後 return  jsonResult, 給我 groovy code','result','GROOVY','qwen2.5-coder','admin','2024-11-04 20:44:54','admin','2024-11-07 22:39:56'),('a34e225a-9d17-11ef-a316-d9ccce9e80b5','PP02','產生echarts 圖表','','請將以下json資料, 使用 echarts 產生 pie, bar, line圖, 各項目標題為 F_NAMEC + \"(\" WATER_SOURCES_ID + \")\" 個項目的實際值為 QUANTITY \njson資料如下:\n```json\n $P{previousInvokeResult}\n```\n給我 html code','','HTML','gemma2','admin','2024-11-07 22:50:32',NULL,NULL),('b2c08a0b-96be-11ef-83bb-cd48be50aaf1','test003','套用 bootstrap css','','把以下的html table內容套上bootstrap css , 給我修改後的html code\n```html \n$P{previousMessage}\n```','','HTML','gemma2','admin','2024-10-30 20:58:45','admin','2024-10-30 21:02:48');
...
INSERT INTO `tb_orrs_command_prompt` VALUES ('0873355c-96cb-11ef-b6c8-d5b873cf6d3a','test002',0,'You are a professional json data master','admin','2024-10-30 22:27:03',NULL,NULL),('087446cd-96cb-11ef-b6c8-8d02018dd451','test002',1,'把 json 資料列 用 html <tr> <td> 顯示出來 , 第一列標題列欄位分別為 json資料第一列的key ','admin','2024-10-30 22:27:03',NULL,NULL),('28a1ac69-9d16-11ef-a316-d1bf2976897d','PP01',0,'遵守: 1.需要 import 的 package class 放在groovy程式碼上方; 2.建議需要 import java.sql.*; 3. 程式碼不要產在自訂義function中','admin','2024-11-07 22:39:56',NULL,NULL),('28a1fa8a-9d16-11ef-a316-df760ee8ce22','PP01',1,'jdbc driver class 為 com.microsoft.sqlserver.jdbc.SQLServerDriver 所以JDBC url 開頭因該為 jdbc:sqlserver:// ,  jdbc url 加上 encrypt=false; 參數','admin','2024-11-07 22:39:56',NULL,NULL),('28a248ab-9d16-11ef-a316-bdfdea54a287','PP01',2,'json library 是 com.fasterxml.jackson','admin','2024-11-07 22:39:56',NULL,NULL),('28a248ac-9d16-11ef-a316-39563c0c00cb','PP01',3,'sql server IP位置 127.0.0.1 , 服務 port 號是 1433 資料庫檔案 NSC_B21 ','admin','2024-11-07 22:39:56',NULL,NULL),('28a248ad-9d16-11ef-a316-8da45c8fbd4e','PP01',4,'你必須遵守: a. 不要把結果用 print 或 println 方式輸出, 請用 return 方式輸出值 , b. 不需要給 implementation , maven, gradle 資訊','admin','2024-11-07 22:39:56',NULL,NULL),('43340fec-96bf-11ef-9d6e-e50e7b144555','test003',0,'bootstrap css CDN base 位置 https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css','admin','2024-10-30 21:02:48',NULL,NULL),('4334fa4d-96bf-11ef-9d6e-eda76a4c8044','test003',1,'bootstrap js CDN base 位置 https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js','admin','2024-10-30 21:02:48',NULL,NULL),('a34e978b-9d17-11ef-a316-b39cd882583a','PP02',0,'echarts 版本請用 5.3.3 ','admin','2024-11-07 22:50:32',NULL,NULL),('a34ee5ac-9d17-11ef-a316-2f1c92152461','PP02',1,'echarts 5.3.3 CDN 位置 https://cdnjs.cloudflare.com/ajax/libs/echarts/5.3.3/echarts.min.js','admin','2024-11-07 22:50:32',NULL,NULL),('b546be67-99b4-11ef-8fda-01540155068c','test001',0,'jdbc driver class 為 org.mariadb.jdbc.Drive 所以JDBC url 開頭因該為 jdbc:mariadb , 建議需要 import java.sql.*','admin','2024-11-03 15:24:48',NULL,NULL),('b5475aa8-99b4-11ef-8fda-3d3120fe9b1a','test001',1,'json library 是 com.fasterxml.jackson 不需要給 implementation 資訊','admin','2024-11-03 15:24:48',NULL,NULL),('b547f6e9-99b4-11ef-8fda-237f9938c5e5','test001',2,'mariadb IP位置 127.0.0.1 , 服務 port 號是 3306 資料庫檔案 orrs ','admin','2024-11-03 15:24:48',NULL,NULL),('b5492f6a-99b4-11ef-8fda-c542e2f71035','test001',3,'需要 import 的 package 放在groovy程式碼上方','admin','2024-11-03 15:24:48',NULL,NULL);
...
INSERT INTO `tb_orrs_task` VALUES ('48c57d5e-8fa6-11ef-92b7-397a3006944e','task01','測試任務01','for test!','0 23 2 * * ?','Y','admin','2024-10-21 20:16:22','admin','2024-11-02 21:24:06'),('7590bed3-9aab-11ef-b609-854d67a445fb','task02','我的水量拆分檔任務','水量資料','0 52 22 * * ?','Y','admin','2024-11-04 20:51:07','admin','2024-11-07 22:51:46');
...
INSERT INTO `tb_orrs_task_cmd` VALUES ('bc730344-991d-11ef-8117-03dd530514de','task01','test001',0,'Y','admin','2024-11-02 21:24:06',NULL,NULL),('bc73eda5-991d-11ef-8117-a10d59bb3594','task01','test002',1,'Y','admin','2024-11-02 21:24:06',NULL,NULL),('bc7489e6-991d-11ef-8117-b9165cb2c430','task01','test003',2,'Y','admin','2024-11-02 21:24:06',NULL,NULL),('cfaa55f4-9d17-11ef-a316-53a4548e67ce','task02','PP01',0,'Y','admin','2024-11-07 22:51:46',NULL,NULL),('cfaaa415-9d17-11ef-a316-f700dc1f66be','task02','PP02',1,'Y','admin','2024-11-07 22:51:46',NULL,NULL);
...
-- Dump completed on 2024-11-09 14:25:28
